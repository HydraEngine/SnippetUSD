...
public:
    ...
    bool IsEnabledTestPresentOutput() const { return PresentComposite() || PresentDisabled(); }
    ...
    CameraUtilConformWindowPolicy const& GetWindowPolicy() const { return _windowPolicy; }
    ...
    bool WriteToFile(UsdImagingGLEngine* engine, TfToken const& aovName, std::string const& filename);

    // Helper method to write an output image from the specified AOV.
    static bool WriteAovToFile(UsdImagingGLEngine* engine, TfToken const& aovName, std::string const& filename);
...
protected:
    ...
    HdRenderIndex* _GetRenderIndex(UsdImagingGLEngine* engine) { return engine->_GetRenderIndex(); }
