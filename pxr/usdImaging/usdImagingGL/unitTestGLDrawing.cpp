...
static void UsdImagingGL_UnitTestHelper_InitPlugins() {
    ...
    std::string pluginDir = TfStringCatPaths(testDir, "UsdImagingPlugins/lib/UsdImagingTest.framework/Resources");
...
UsdImagingGL_UnitTestWindow::UsdImagingGL_UnitTestWindow(UsdImagingGL_UnitTestGLDrawing* unitTest, int w, int h)
...
bool UsdImagingGL_UnitTestWindow::WriteToFile(std::string const& attachment, std::string const& filename) {
    ...
    if (isBound) _drawTarget->Unbind();

    bool result = _drawTarget->WriteToFile(attachment, filename);

    if (isBound) _drawTarget->Bind();
    return result;
}
...
bool UsdImagingGL_UnitTestGLDrawing::WriteToFile(UsdImagingGLEngine* engine,
    ...
    if (IsEnabledTestPresentOutput()) {
        ...
        return UsdImagingGL_UnitTestGLDrawing::WriteAovToFile(engine, aovName, filename);
    }
}

bool UsdImagingGL_UnitTestGLDrawing::WriteAovToFile(UsdImagingGLEngine* engine,
                                                    TfToken const& aovName,
                                                    std::string const& filename) {
    ...
    TextureBuffer buffer = HdStTextureUtils::HgiTextureReadback<uint16_t>(hgi, texture, &bufferSize);
...
void UsdImagingGL_UnitTestGLDrawing::RunTest(int argc, char* argv[]) {
    ...
    _dataWindow = GfRect2i(GfVec2i(args.dataWindow[0], args.dataWindow[1]), args.dataWindow[2], args.dataWindow[3]);
