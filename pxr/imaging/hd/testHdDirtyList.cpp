...
static HdReprSelector wireOnSurfWithPoints(HdReprTokens->wireOnSurf, HdReprTokens->disabled, HdReprTokens->points);
...
static bool BasicTest() {
    ...
    //    So, all the rprims should be in the dirty list.
    {
        ...
        dl.UpdateRenderTagsAndReprSelectors(TfTokenVector(/*empty render tags*/), HdReprSelectorVector({surface}));
    ...
    //    back, the dirty list will be rebuilt to just the varying rprims.
    {
        ...
        dl.UpdateRenderTagsAndReprSelectors(TfTokenVector(/*empty render tags*/), HdReprSelectorVector({wireOnSurf}));
        _VerifyDirtyListSize(&dl, numGeometryPrims + numGuidePrims);
    ...
    // 3. Update the render tags.
    {
        ...
        dl.UpdateRenderTagsAndReprSelectors(TfTokenVector({HdRenderTagTokens->guide}), HdReprSelectorVector({surface}));
...
TEST(TestHydra, test_dirty_list) {
    TfErrorMark mark;
