...
public:
    ...
    HdExtComputationPrimvarDescriptorVector GetExtComputationPrimvarDescriptors(
        ...
        if (id == pathA && interpolationMode == HdInterpolationConstant) {
            ...
            primvars.emplace_back(primvarName, HdInterpolationConstant, HdPrimvarRoleTokens->none, compA,
                                  compOutputName, valueType);
            return primvars;
        }
    ...
    virtual void InvokeExtComputation(SdfPath const& computationId, HdExtComputationContext* context) override {
        if (computationId != compA) {
            return;
        }
...
public:
    virtual HdResourceRegistrySharedPtr GetResourceRegistry() const override { return nullptr; }

    virtual HdRenderPassSharedPtr CreateRenderPass(HdRenderIndex* index, HdRprimCollection const& collection) override {
        return nullptr;
    }

    virtual HdInstancer* CreateInstancer(HdSceneDelegate* delegate, SdfPath const& id) override { return nullptr; }
    virtual void DestroyInstancer(HdInstancer* instancer) override {}

    virtual HdRprim* CreateRprim(TfToken const& typeId, SdfPath const& rprimId) override { return nullptr; }
    ...
    virtual HdSprim* CreateFallbackSprim(TfToken const& typeId) override { return nullptr; }
    virtual void DestroySprim(HdSprim* sprim) override { delete sprim; }

    virtual HdBprim* CreateBprim(TfToken const& typeId, SdfPath const& bprimId) override { return nullptr; }
    virtual HdBprim* CreateFallbackBprim(TfToken const& typeId) override { return nullptr; }
    virtual void DestroyBprim(HdBprim* bPrim) override {}

    virtual void CommitResources(HdChangeTracker* tracker) override {}

    virtual const TfTokenVector& GetSupportedRprimTypes() const override { return _emptyTypes; }
    virtual const TfTokenVector& GetSupportedSprimTypes() const override { return _sprimTypes; }
    virtual const TfTokenVector& GetSupportedBprimTypes() const override { return _emptyTypes; }
};
...
void RunTest() {
    ...
    HdExtComputationUtils::SampleComputedPrimvarValues(compPrimvars, &delegate, maxSamples, &valueStore);

    if (valueStore.size() != 1) {
        TF_RUNTIME_ERROR("Incorrect number of computed primvars %d", static_cast<int>(valueStore.size()));
        return;
    }
...
#define CHECK_SAMPLE(index, time, val)                                                                    \
    ...
    if (pvSamples.values[index].Get<double>() != val) {                                                   \
        TF_RUNTIME_ERROR("Unexpected sample value %f vs %f", pvSamples.values[index].Get<double>(), val); \
        return;                                                                                           \
    }
...
TEST(TestHydra, test_ext_computation_utils) {
    TfErrorMark mark;
