...
#include "../materialNetworkShader.h"
...
void My_TestGLDrawing::InitTest() {
    ...
    _textureHandleRegistry = std::make_unique<HdSt_TextureHandleRegistry>(_hdStRegistry.get());
}
...
class My_DynamicUvTextureImplementation : public HdStDynamicUvTextureImplementation {
public:
...
HdStDynamicUvTextureImplementation* My_SubtextureIdentifier::GetTextureImplementation() const {
...
void My_TestGLDrawing::OffscreenTest() {
    ...
    HdStShaderCodeSharedPtr const shader = std::make_shared<HdSt_MaterialNetworkShader>();

    // Create texture handle using our own subtexture identifier.
    // Low blue component.
    //
    HdStTextureHandleSharedPtr const textureHandle1 = _textureHandleRegistry->AllocateTextureHandle(
            HdStTextureIdentifier(TfToken(), std::make_unique<My_SubtextureIdentifier>(90)), HdStTextureType::Uv,
            HdSamplerParameters(HdWrapRepeat, HdWrapRepeat, HdWrapClamp, HdMinFilterNearest, HdMagFilterNearest),
            /* memoryRequest = */ 2000, shader);

    // High blue component.
    HdStTextureHandleSharedPtr const textureHandle2 = _textureHandleRegistry->AllocateTextureHandle(
            HdStTextureIdentifier(TfToken(), std::make_unique<My_SubtextureIdentifier>(230)), HdStTextureType::Uv,
            HdSamplerParameters(HdWrapRepeat, HdWrapRepeat, HdWrapClamp, HdMinFilterNearest, HdMagFilterNearest),
            /* memoryRequest = */ 2000, shader);
...

    {
        ...
        _driver->Draw(dstTexture, uvTextureObject->GetTexture(), uvSamplerObject->GetSampler());
