...
public:
    ...
    void SetCamera(GfMatrix4d const& viewMatrix, GfMatrix4d const& projectionMatrix, CameraUtilFraming const& framing);
    ...
    void SetClearColor(GfVec4f const& clearColor) { _clearColor = clearColor; }
...
template <typename SceneDelegate>
void HdSt_TestDriverBase<SceneDelegate>::_SetupSceneDelegate() {
    _renderIndex = HdRenderIndex::New(&_renderDelegate, {&_hgiDriver});
    TF_VERIFY(_renderIndex != nullptr);

    _sceneDelegate = new SceneDelegate(_renderIndex, SdfPath::AbsoluteRootPath());
}

template <typename SceneDelegate>
void HdSt_TestDriverBase<SceneDelegate>::_Init() {
    _Init(HdReprSelector(HdReprTokens->smoothHull));
}

template <typename SceneDelegate>
void HdSt_TestDriverBase<SceneDelegate>::_Init(HdReprSelector const& reprSelector) {
    _SetupSceneDelegate();
...
template <typename SceneDelegate>
void HdSt_TestDriverBase<SceneDelegate>::SetCamera(GfMatrix4d const& viewMatrix,
    ...
    _sceneDelegate->UpdateCamera(_cameraId, HdCameraTokens->projection, VtValue(_ToHd(cam.GetProjection())));
    _sceneDelegate->UpdateCamera(_cameraId, HdCameraTokens->focalLength,
                                 VtValue(cam.GetFocalLength() * float(GfCamera::FOCAL_LENGTH_UNIT)));
    _sceneDelegate->UpdateCamera(_cameraId, HdCameraTokens->horizontalAperture,
                                 VtValue(cam.GetHorizontalAperture() * float(GfCamera::APERTURE_UNIT)));
    _sceneDelegate->UpdateCamera(_cameraId, HdCameraTokens->verticalAperture,
                                 VtValue(cam.GetVerticalAperture() * float(GfCamera::APERTURE_UNIT)));
    _sceneDelegate->UpdateCamera(_cameraId, HdCameraTokens->horizontalApertureOffset,
                                 VtValue(cam.GetHorizontalApertureOffset() * float(GfCamera::APERTURE_UNIT)));
    _sceneDelegate->UpdateCamera(_cameraId, HdCameraTokens->verticalApertureOffset,
                                 VtValue(cam.GetVerticalApertureOffset() * float(GfCamera::APERTURE_UNIT)));
    _sceneDelegate->UpdateCamera(_cameraId, HdCameraTokens->clippingRange, VtValue(cam.GetClippingRange()));
    ...
    const HdCamera* const camera =
            dynamic_cast<HdCamera const*>(_renderIndex->GetSprim(HdPrimTypeTokens->camera, _cameraId));
    TF_VERIFY(camera);
...
template <typename SceneDelegate>
void HdSt_TestDriverBase<SceneDelegate>::SetCameraClipPlanes(std::vector<GfVec4d> const& clipPlanes) {
    _sceneDelegate->UpdateCamera(_cameraId, HdCameraTokens->clipPlanes, VtValue(clipPlanes));
}

template <typename SceneDelegate>
void HdSt_TestDriverBase<SceneDelegate>::SetCullStyle(HdCullStyle cullStyle) {
    for (const HdStRenderPassStateSharedPtr& renderPassState : _renderPassStates) {
...
template <typename SceneDelegate>
void HdSt_TestDriverBase<SceneDelegate>::SetRepr(HdReprSelector const& reprSelector) {
    _collection.SetReprSelector(reprSelector);
...
static TfTokenVector _aovOutputs{HdAovTokens->color, HdAovTokens->depth};

template <typename SceneDelegate>
SdfPath HdSt_TestDriverBase<SceneDelegate>::_GetAovPath(TfToken const& aov) const {
...
template <typename SceneDelegate>
void HdSt_TestDriverBase<SceneDelegate>::SetupAovs(int width, int height) {
    if (_aovBindings.empty()) {
        ...
        for (size_t i = 0; i < _aovOutputs.size(); i++) {
            ...
            HdAovDescriptor aovDesc = _renderDelegate.GetDefaultAovDescriptor(_aovOutputs[i]);
            ...
            binding.renderBuffer =
                    dynamic_cast<HdRenderBuffer*>(_renderIndex->GetBprim(HdPrimTypeTokens->renderBuffer, aovId));
...
template <typename SceneDelegate>
bool HdSt_TestDriverBase<SceneDelegate>::WriteToFile(std::string const& attachment, std::string const& filename) {
    ...
    if (!image) {
        TF_RUNTIME_ERROR("Failed to open image for writing %s", filename.c_str());
        return false;
    }
...
template <typename SceneDelegate>
void HdSt_TestDriverBase<SceneDelegate>::UpdateAovDimensions(int width, int height) {
...
template <typename SceneDelegate>
void HdSt_TestDriverBase<SceneDelegate>::Present(int width, int height, uint32_t framebuffer) {
    ...
    _interop.TransferToApp(_hgi.get(), colorTexture,
                           /*srcDepth*/ HgiTextureHandle(), HgiTokens->OpenGL, VtValue(framebuffer),
...
public:
    ...
    HDST_API
    const HdStRenderPassStateSharedPtr& GetRenderPassState() const { return _renderPassStates[0]; }
...
///
using HdSt_TestLightingShaderSharedPtr = std::shared_ptr<class HdSt_TestLightingShader>;
...
public:
    ...
    HDST_API
    bool WriteToFile(HgiTextureHandle const& dstTexture, std::string filename) const;
...
private:
    ...
    bool _CreateTextureBindings(HgiTextureHandle const& textureHandle, HgiSamplerHandle const& samplerHandle);
