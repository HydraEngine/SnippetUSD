...
public:
    std::string Resolve(const std::string& resolvedPackagePath, const std::string& packagedPath) override {
        TF_AXIOM(TfStringEndsWith(resolvedPackagePath, ".package"));
        return packagedPath;
    }

    std::shared_ptr<ArAsset> OpenAsset(const std::string& resolvedPackagePath,
